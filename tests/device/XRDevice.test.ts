...
describe('XRDevice', () => {
  ...
  test('should handle primary input mode changes', () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      'primary input mode can only be "controller" or "hand"',
    );
  ...
  test('should handle property getters and setters', () => {
    ...
    const stereoWidth = Math.round(1024 * (2 - 0.85));
  ...
  test('computes stereo render target width using overlap', () => {
    const device = new XRDevice(mockDeviceConfig, {
    ...
    const expectedStereoWidth = Math.round(1832 * (2 - 0.86));
    expect(device.getRecommendedRenderTargetSize(true)).toEqual({
      width: expectedStereoWidth,
      height: 1920,
    });
