...
- RemoteControlInterface: world/scene queries (get_world_state, get_objects), set_hand_pose, a runtime capability manifest (listMethods()/describe()), an opt-in transport bridge (connectTransport), and a fix for get_session_status reporting sessionMode undefined.
